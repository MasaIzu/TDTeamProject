...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* EnemyBoostParticle::device = nullptr;
ID3D12GraphicsCommandList* EnemyBoostParticle::cmdList = nullptr;
ComPtr<ID3D12RootSignature> EnemyBoostParticle::rootsignature;
ComPtr<ID3D12RootSignature> EnemyBoostParticle::rootSignature;//コンピュートシェーダー用
ComPtr<ID3D12PipelineState> EnemyBoostParticle::pipelinestate;
ComPtr<ID3D12PipelineState> EnemyBoostParticle::pipelineState;//コンピュートシェーダー用

std::unordered_map<std::string,ComPtr<ID3D12PipelineState>> EnemyBoostParticle::m_pipelines;
ComPtr<ID3D12DescriptorHeap> EnemyBoostParticle::m_cbvSrvUavHeap;

const std::string EnemyBoostParticle::PSO_DEFAULT = "PSO_DEFAULT";
const std::string EnemyBoostParticle::PSO_CS_INIT = "PSO_CS_INIT";
const std::string EnemyBoostParticle::PSO_CS_EMIT = "PSO_CS_EMIT";
const std::string EnemyBoostParticle::PSO_CS_UPDATE = "PSO_CS_UPDATE";
const std::string EnemyBoostParticle::PSO_DRAW_PARTICLE = "PSO_DRAW_PARTICLE";
const std::string EnemyBoostParticle::PSO_DRAW_PARTICLE_USE_TEX = "PSO_DRAW_PARTICLE_USE_TEX";

UINT EnemyBoostParticle::m_incrementSize;

UINT EnemyBoostParticle::m_cbvSrvUavDescriptorSize = 0;

void EnemyBoostParticle::StaticInitialize(ID3D12Device* Device)
{
	// nullptrチェック
	assert(Device);

	EnemyBoostParticle::device = Device;
...
void EnemyBoostParticle::StaticFinalize()
{

}

void EnemyBoostParticle::PreDraw(ID3D12GraphicsCommandList* commandList)
{
	// PreDrawとPostDrawがペアで呼ばれていなければエラー
	assert(EnemyBoostParticle::cmdList == nullptr);

	// コマンドリストをセット
	EnemyBoostParticle::cmdList = commandList;
...
void EnemyBoostParticle::PostDraw()
{
	// コマンドリストを解除
	EnemyBoostParticle::cmdList = nullptr;
}

void EnemyBoostParticle::InitializeGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/Particle/EnemyBoost/EnemyBoostParticleVS.hlsl",	// シェーダファイル名
		nullptr,
...
// errorBlobからエラー内容をstring型にコピー
	...
	result = D3DCompileFromFile(
		L"Resources/Shaders/Particle/EnemyBoost/EnemyBoostParticleGS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
// errorBlobからエラー内容をstring型にコピー
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/Shaders/Particle/EnemyBoost/EnemyBoostParticlePS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
// errorBlobからエラー内容をstring型にコピー
	...
	// コンピュートシェーダーのコンパイル
	D3DCompileFromFile(
		L"Resources/Shaders/Particle/EnemyBoost/EnemyBoostParticleCS.hlsl",
...
void EnemyBoostParticle::InitializeVerticeBuff()
{
...
void EnemyBoostParticle::SetTextureHandle(uint32_t textureHandle) {
	textureHandle_ = textureHandle;
}

void EnemyBoostParticle::Initialize(uint32_t ParticleCount)
{
...
void EnemyBoostParticle::Draw(const ViewProjection& view)
{
	...
	// nullptrチェック
	assert(device);
	assert(EnemyBoostParticle::cmdList);
...
void EnemyBoostParticle::CSUpdate(ID3D12GraphicsCommandList* commandList,const MyStruct::EnemyBoostPos& boostPos,const float& boostPower/*,const uint32_t& shot*/)
{

	ID3D12DescriptorHeap* ppHeaps[ ] = { m_cbvSrvUavHeap.Get() };
	commandList->SetDescriptorHeaps(_countof(ppHeaps),ppHeaps);
	for ( uint32_t i = 0; i < static_cast< uint32_t >( Numbers::Two ); i++ )
	{
...
void EnemyBoostParticle::CopyData()
{
