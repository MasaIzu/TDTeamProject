...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* ParticleCS::device = nullptr;
ID3D12GraphicsCommandList* ParticleCS::cmdList = nullptr;
ComPtr<ID3D12RootSignature> ParticleCS::rootsignature;
ComPtr<ID3D12RootSignature> ParticleCS::rootSignature;//コンピュートシェーダー用
ComPtr<ID3D12PipelineState> ParticleCS::pipelinestate;
ComPtr<ID3D12PipelineState> ParticleCS::pipelineState;//コンピュートシェーダー用

std::unordered_map<std::string, ComPtr<ID3D12PipelineState>> ParticleCS::m_pipelines;
ComPtr<ID3D12DescriptorHeap> ParticleCS::m_cbvSrvUavHeap;

const std::string ParticleCS::PSO_DEFAULT = "PSO_DEFAULT";
const std::string ParticleCS::PSO_CS_INIT = "PSO_CS_INIT";
const std::string ParticleCS::PSO_CS_EMIT = "PSO_CS_EMIT";
const std::string ParticleCS::PSO_CS_UPDATE = "PSO_CS_UPDATE";
const std::string ParticleCS::PSO_DRAW_PARTICLE = "PSO_DRAW_PARTICLE";
const std::string ParticleCS::PSO_DRAW_PARTICLE_USE_TEX = "PSO_DRAW_PARTICLE_USE_TEX";

UINT ParticleCS::m_incrementSize;

UINT ParticleCS::m_cbvSrvUavDescriptorSize = 0;
...
void ParticleCS::StaticFinalize()
{
...
{
	// コマンドリストを解除
	ParticleCS::cmdList = nullptr;
}

void ParticleCS::InitializeGraphicsPipeline()
{
...
void ParticleCS::InitializeVerticeBuff()
{
...
void ParticleCS::SetTextureHandle(uint32_t textureHandle) {
	textureHandle_ = textureHandle;
}

void ParticleCS::Initialize(uint32_t ParticleCount)
{
	HRESULT result;
...
void ParticleCS::Draw(const ViewProjection& view)
{
	HRESULT result;
	// 定数バッファへデータ転送
