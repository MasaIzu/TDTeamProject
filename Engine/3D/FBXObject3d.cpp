...
ComPtr<ID3D12RootSignature> FBXObject3d::lightRootsignature;
ComPtr<ID3D12PipelineState> FBXObject3d::lightPipelinestate;
...
void FBXObject3d::CreateLightGraphicsPipeline()
{
	HRESULT result = S_FALSE;
	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
	ComPtr<ID3DBlob> psBlob;    // ピクセルシェーダオブジェクト
	...
	// 頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/shaders/LightFBXVS.hlsl",    // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main","vs_5_0",    // エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
...
// errorBlobからエラー内容をstring型にコピー
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/shaders/LightFBXPS.hlsl",    // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
		"main","ps_5_0",    // エントリーポイント名、シェーダーモデル指定
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用設定
...
// errorBlobからエラー内容をstring型にコピー
	...
	result = D3DX12SerializeVersionedRootSignature(&rootSignatureDesc,D3D_ROOT_SIGNATURE_VERSION_1_0,&rootSigBlob,&errorBlob);
	// ルートシグネチャの生成
	result = device->CreateRootSignature(0,rootSigBlob->GetBufferPointer(),rootSigBlob->GetBufferSize(),IID_PPV_ARGS(lightRootsignature.ReleaseAndGetAddressOf()));
	if ( FAILED(result) )
	{
		assert(0);
	}

	gpipeline.pRootSignature = lightRootsignature.Get();

	// グラフィックスパイプラインの生成
	result = device->CreateGraphicsPipelineState(&gpipeline,IID_PPV_ARGS(lightPipelinestate.ReleaseAndGetAddressOf()));
	if ( FAILED(result) )
	{
		assert(0);
	}
...
void FBXObject3d::Update(const uint32_t& animationNum,const FbxTime& nowCount,const FbxTime& maxCount)
{
	HRESULT result;
	std::vector<FBXModel::Bone>& bones = fbxmodel->GetBones();
...
void FBXObject3d::Draw(const WorldTransform& worldTransform,const ViewProjection& viewProjection,const ViewProjection& lightViewProjection)
{
	// モデルの割り当てがなければ描画しない
	if ( fbxmodel == nullptr )
	{
...
void FBXObject3d::ShadowDraw(const WorldTransform& worldTransform,const ViewProjection& viewProjection) {
	...
	// パイプラインステートの設定
	DirectXCore::GetInstance()->GetCommandList()->SetPipelineState(lightPipelinestate.Get());
	// ルートシグネチャの設定
	DirectXCore::GetInstance()->GetCommandList()->SetGraphicsRootSignature(lightRootsignature.Get());
	// プリミティブ形状を設定
	DirectXCore::GetInstance()->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	// CBVをセット（ワールド行列）
	DirectXCore::GetInstance()->GetCommandList()->SetGraphicsRootConstantBufferView(0,worldTransform.constBuff_->GetGPUVirtualAddress());
	...
	DirectXCore::GetInstance()->GetCommandList()->SetGraphicsRootConstantBufferView(5,constBuffSkin->GetGPUVirtualAddress());
	// モデル描画
	fbxmodel->ShadowDraw(DirectXCore::GetInstance()->GetCommandList());
}
